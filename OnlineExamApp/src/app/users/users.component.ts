...
export class UsersComponent implements OnInit, PipeTransform {
  ...
  public technologyTypes: string[] = [".Net Developer", "Angular Developer","Fullstack Developer","Rust Developer","PHP Developer","Python Developer","VueJs","Tester","Scrum Master","Devops Engineer","Agu"];
 public userStatuses:string[]=["Interview Scheduled","Interview Done","Evaluation Submitted","Passed Tech Round","Failed Tech Round","Offer Accepted","Offer Rejected"];
  closeResult = '';
  loading: boolean;
  submitted = false;
  ...
  constructor(private http: HttpClient, private router: Router, private activatedroute: ActivatedRoute, private formBuilder: FormBuilder, private modalService: NgbModal) {
  ...
  getQuestions() {
    ...
    this.http.get<any>("https://ariqtonlineexamapi.azurewebsites.net/api/Question/technology/" + technologyid).subscribe(res => {
      ...
      else if (this.selecteduser.experience > 2 && this.selecteduser.experience <= 5) {
        for (var i = 0; i < res.length; i++) {
      ...
      this.http.get<any>('https://ariqtonlineexamapi.azurewebsites.net/api/Answer/' + this.selecteduser.userId).subscribe(data => {
        ...
        for (var i = 0; i<this.quiz.questions.length; i++) {
          ...
          for (var k = 0; k < answers.length; k++) {
            if (answers[k].questionId == this.quiz.questions[i].questionId) {
              if (this.quiz.questions[i].questionType == 1) {
                for (var j = 0; j < this.quiz.questions[i].choices.length; j++) {

                  if (this.quiz.questions[i].choices[j].choiceId == answers[k].choiceId) {
      ...
      var percentage=(this.TotalScore/this.getNumberOfQuestions)*100;
  ...
  getUserList() {
    ...
    this.http.get<any>('https://ariqtonlineexamappapi.azurewebsites.net/api/User').subscribe(data => {
  ...
  getEvaluationFormDetails(candidatedata){
    ...
    this.http.get<any>("https://ariqtonlineexamappapi.azurewebsites.net/api/InterviewerReviewForm/" + candidatedata.userId).subscribe(res => {
...
for(var i=0;i<res.length;i++)
{
  if(res[i].reviewTypeId==1)
  {
    ...
    (<any>(document.getElementById('txtrating1'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==2)
  {
    this.rating2=res[i].rating;
    (<any>(document.getElementById('txtrating2'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==3)
  {
    this.rating3=res[i].rating;
    (<any>(document.getElementById('txtrating3'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==4)
  {
    this.rating4=res[i].rating;
    (<any>(document.getElementById('txtrating4'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==5)
  {
    this.rating5=res[i].rating;
    (<any>(document.getElementById('txtrating5'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==6)
  {
    this.rating6=res[i].rating;
    (<any>(document.getElementById('txtrating6'))).value = res[i].comments;
  }
  if(res[i].reviewTypeId==7)
  {
    this.rating7=res[i].rating;
    (<any>(document.getElementById('txtrating7'))).value = res[i].comments;
  }
}
  ...
  download(candidate) {
    ...
    window.location.href = 'https://onlineexamresumes.blob.core.windows.net/interviewcandidates/' + candidate.uploadResume;
  }
  ...
  isCorrect(question: examQuestion) {
    ...
    var result = question.choices.every(x => x.selected === x.isCorrect) ? 'Correct' : 'Wrong';
  ...
  sendMail(result) {
    ...
    this.http.post<any>('https://ariqtonlineexamappapi.azurewebsites.net/api/User/sendemail', result).subscribe(data => {
  ...
  sendEvaluation(result) {
    ...
    this.http.post<any>('https://ariqtonlineexamappapi.azurewebsites.net/api/User/sendevaluation', result).subscribe(data => {
  ...
  Search() {
    ...
    if (this.userName != "" || this.mobileNo != "" || this.email != "" || this.experience != "") {
      this.userTableData2 = this.userTableData.filter(res => {
        return res.userName.toLocaleLowerCase().match(this.userName.toLocaleLowerCase()) || res.mobileNo.toLocaleLowerCase().match(this.mobileNo.toLocaleLowerCase() || res.email.toLocaleLowerCase().match(this.email.toLocaleLowerCase()) || res.experience.toLocaleLowerCase().match(this.experience.toLocaleLowerCase()));
      });
    }
  ...
  viewCompleteDeatils(viewdeatils, row) {
    ...
    this.loading=false;
    this.modalService.open(viewdeatils, {
    ...
    (<any>(document.getElementById('viewexpectedctc'))).value = row.expectedSalary;
  ...
  editCompleteDeatils(editdeatils,row)
  {
    ...
    this.modalService.open(editdeatils, {
      ariaLabelledBy: 'modal-basic-title'
    }).result.then((result) => {
      this.closeResult = `Closed with: ${result}`;
    }, (reason) => {
    ...
    (<any>(document.getElementById('txteditinterviewer'))).value = row.interviewer;
    (<any>(document.getElementById('txteditExperience'))).value = row.experience;
    (<any>(document.getElementById('txteditCurrentOrganization'))).value = row.currentOrgnization;
    (<any>(document.getElementById('txteditCurrentCtc'))).value = row.currentCTC;
    (<any>(document.getElementById('txteditExpectedCtc'))).value = row.expectedSalary;
    (<any>(document.getElementById('txteditNotice'))).value = row.noticePeriod;
    (<any>(document.getElementById('txteditContactedby'))).value = row.contactedby;
    if(row.uploadResume!="")
    {
  ...
  colfilter(event)
  {
    ...
    {
    this.userTableData2 = this.userTableData.filter(res => {
      return res.email.toLocaleLowerCase().match(this.emailaddr.toLocaleLowerCase());
    });
  }
  else{
    this.getUserList();
...
debugger
    ...
    var userstatus = e.options[e.selectedIndex].text;
      ...
      var currentOrg = (<any>(document.getElementById('txteditCurrentOrganization'))).value;
      var currentCtc = (<any>(document.getElementById('txteditCurrentCtc'))).value;
      var expectedCtc = (<any>(document.getElementById('txteditExpectedCtc'))).value;
      var noticeperiod = (<any>(document.getElementById('txteditNotice'))).value;
      var contactedby = (<any>(document.getElementById('txteditContactedby'))).value;
      var myDate = new Date().toISOString();
      var datetimevalue = myDate.substring(0, myDate.length - 1);
debugger
      var result = {UserId:this.getUserId, UserName: name, MobileNo: mobile, Email: email, Experience: experiance, CreatedDate: datetimevalue, Technology: technology, Contactedby: contactedby, CurrentCTC: currentCtc, CurrentOrgnization: currentOrg, ExpectedSalary: expectedCtc, NoticePeriod: noticeperiod,Interviewer:interviewr,UserStatus:userstatus };
      this.http.put<any>('https://ariqtonlineexamappapi.azurewebsites.net/api/User/UpdateUsers', result).subscribe(data => {
  ...
  scheduleInterview(row)
  {
    if(row.examStatus==3)
    {
      ...
      this.router.navigate(['/schedule-interview'], {queryParams: {rowdata: result},skipLocationChange: true, replaceUrl: false});
    }
    else
    {
      alert("Hehe don't do smart ha..\nSchedule only for those who are complete the Exam...");
    }
  }
}
