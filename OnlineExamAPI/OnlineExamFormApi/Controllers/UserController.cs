...
namespace OnlineExamAPI.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        {
            user.IsReviewGiven = false;
            user.UserStatus = "Interview Scheduled";
            if (user == null)
            {
        ...
        [HttpPost("sendevaluation")]
        public IActionResult SendEvaluation(Users userDetails)
        {
            var user = _onlineExam.GetByReference(userDetails.UserReferenceId);
            userDetails.Interviewer = user.Interviewer;
            string body = PopulateEvaluationFormBody(userDetails);
            _emailService.SendEvaluationMail(userDetails, body);
            _onlineExam.Update(user);
            return Ok(true);
        }
        ...
        private string PopulateEvaluationFormBody(Users userDetails)
        {
            string body = string.Empty;
            string fileFullPath = $"{Directory.GetCurrentDirectory()}/wwwroot/EmailTemplates/InterviewEvaluation.html";
            using (StreamReader reader = System.IO.File.OpenText(fileFullPath))
            {
                body = reader.ReadToEnd();
            }
            body = body.Replace("{url}", "https://ariqtexamportal.azurewebsites.net/interview-evaluation?id=" + userDetails.UserReferenceId);
            body = body.Replace("{interviewer}", userDetails.Interviewer);
        ...
        private string PopulateBody(Users userDetails)
        {
            ...
            body = body.Replace("{url}", "https://ariqtexamportal.azurewebsites.net/app-login-user?id=" + userDetails.UserReferenceId);
            body = body.Replace("{userName}", userDetails.UserName);
            return body;
        }
